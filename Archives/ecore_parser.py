...
class EcoreToGaml:
    ...
    def getInheritanceLinks(self): # Get children and parents classes from inheritance links.
        ascendants = {}
        descendants = {}
    ...
    def translate(self):
        ascendants, descendants = self.getInheritanceLinks()
        species = []
    ...
    def getParentClass(self, tag): # Get parent class name.
        parent = ''
        spl = [s.strip() for s in tag['eSuperTypes'].split('#//') if (s.strip() != '' and s.strip() != ROOT)] # Echap ROOT tag (hack to found right classes).
...
if __name__== "__main__":
    ecore_tree = EcoreParser('data/resiist.ecore')
